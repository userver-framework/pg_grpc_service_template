...
docker-impl-%:
	docker-compose run --rm pg_grpc_service_template-service make $*

# Build and runs service in docker environment
docker-start-service-debug:
	@docker-compose run -p 8080:8080 --rm pg_grpc_service_template-service make -- --debug-start-in-docker-debug

# Build and runs service in docker environment
docker-start-service:
	@docker-compose run -p 8080:8080 --rm pg_grpc_service_template-service make -- --debug-start-in-docker
